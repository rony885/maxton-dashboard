...
const show = (stepperNode, indexStep, options, done) => {
  ...
  const activeStep = stepper._steps.filter((step) =>
    step.classList.contains(ClassName.ACTIVE)
  );
  const activeContent = stepper._stepsContents.filter((content) =>
    content.classList.contains(ClassName.ACTIVE)
  );
...
export { show, ClassName, customProperty, detectAnimation };
